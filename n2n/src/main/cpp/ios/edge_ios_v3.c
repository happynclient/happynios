...
int start_edge_v3(n2n_edge_status_t* status) {
    ...
    tuntap.upd_socket = cmd->udp_fd;
    tuntap.mgr_socket = cmd->mgr_fd;
    tuntap.sendto_sock = cmd->sendto_sock;
    tuntap.recvfrom = cmd->recvfrom;
    tuntap.tuntap_write = cmd->tuntap_write;
#endif //__IOS_PLATFORM__

    conf.drop_multicast = cmd->drop_multicast == 0 ? 1 : 0;
    ...
    if(cmd->supernode == NULL)
        goto cleanup;
    
    if(strlen(cmd->supernode) == 0)
        goto cleanup;
        
    if(0 == edge_conf_add_supernode(&conf, cmd->supernode))
        traceEvent(TRACE_DEBUG, "Adding supernode[%u] = %s\n", (unsigned int) conf.sn_num, cmd->supernode);
        
    if(cmd->supernode2 && strlen(cmd->supernode2))
        if(0 == edge_conf_add_supernode(&conf, cmd->supernode2))
            traceEvent(TRACE_DEBUG, "Adding supernode[%u] = %s\n", (unsigned int) conf.sn_num, cmd->supernode2);

    if (cmd->ip_netmask && cmd->ip_netmask[0] != '\0')
        strncpy(netmask, cmd->ip_netmask, N2N_NETMASK_STR_SIZE);

    if (cmd->gateway_ip && cmd->gateway_ip[0] != '\0')
        inet_aton(cmd->gateway_ip, &gateway_ip);

    if (cmd->mac_addr && cmd->mac_addr[0] != '\0')
    ...
    if(cmd->device_description && cmd->device_description[0]) {
        conf.dev_desc[sizeof(conf.dev_desc) - 1] = '\0';
        strncpy(conf.dev_desc, cmd->device_description, sizeof(conf.dev_desc) - 1);
    }
